...
func TestRequirementConstructor(t *testing.T) {
	requirementConstructorTests := []struct {
		...
		{"x", NotInOperator, NewStringSet(), false},
		{"x", InOperator, NewStringSet("foo"), true},
		{"x", NotInOperator, NewStringSet("foo"), true},
		{"x", ExistsOperator, nil, true},
		{"1foo", InOperator, NewStringSet("bar"), true},
		{"1234", InOperator, NewStringSet("bar"), true},
...
func TestToString(t *testing.T) {
	...
	toStringTests := []struct {
		...
		{&LabelSelector{
			getRequirement("x", InOperator, NewStringSet("abc", "def"), t),
			getRequirement("y", NotInOperator, NewStringSet("jkl"), t),
		...
		{&LabelSelector{
			...
			"x in (abc,def),", false},
		{&LabelSelector{
			getRequirement("x", NotInOperator, NewStringSet("abc"), t),
			getRequirement("y", InOperator, NewStringSet("jkl", "mno"), t),
			getRequirement("z", NotInOperator, NewStringSet(""), t)},
			"x notin (abc),y in (jkl,mno),z notin ()", true},
		{&LabelSelector{
			getRequirement("x", EqualsOperator, NewStringSet("abc"), t),
			getRequirement("y", DoubleEqualsOperator, NewStringSet("jkl"), t),
			getRequirement("z", NotEqualsOperator, NewStringSet("a"), t)},
...
func TestRequirementLabelSelectorMatching(t *testing.T) {
	...
	labelSelectorMatchingTests := []struct {
		...
		{Set{"x": "foo", "y": "baz"}, &LabelSelector{
			getRequirement("x", InOperator, NewStringSet("foo"), t),
			getRequirement("y", NotInOperator, NewStringSet("alpha"), t),
		}, true},
		{Set{"x": "foo", "y": "baz"}, &LabelSelector{
			getRequirement("x", InOperator, NewStringSet("foo"), t),
			getRequirement("y", InOperator, NewStringSet("alpha"), t),
		}, false},
		{Set{"y": ""}, &LabelSelector{
			getRequirement("x", NotInOperator, NewStringSet(""), t),
			getRequirement("y", ExistsOperator, nil, t),
		}, true},
		{Set{"y": "baz"}, &LabelSelector{
			getRequirement("x", InOperator, NewStringSet(""), t),
		}, false},
	}
...
func TestSetSelectorParser(t *testing.T) {
	setSelectorParserTests := []struct {
		...
		{"this-is-another-dns.domain.com/key-with-dash in (so,what)", LabelSelector{
			getRequirement("this-is-another-dns.domain.com/key-with-dash", InOperator, NewStringSet("so", "what"), t),
		}, true, true},
		{"0.1.2.domain/99 notin (10.10.100.1, tick.tack.clock)", LabelSelector{
			getRequirement("0.1.2.domain/99", NotInOperator, NewStringSet("10.10.100.1", "tick.tack.clock"), t),
		}, true, true},
		{"foo  in	 (abc)", LabelSelector{
			getRequirement("foo", InOperator, NewStringSet("abc"), t),
		}, true, true},
		{"x notin\n (abc)", LabelSelector{
			getRequirement("x", NotInOperator, NewStringSet("abc"), t),
		}, true, true},
		{"x  notin	\t	(abc,def)", LabelSelector{
			getRequirement("x", NotInOperator, NewStringSet("abc", "def"), t),
		}, true, true},
		{"x in (abc,def)", LabelSelector{
			getRequirement("x", InOperator, NewStringSet("abc", "def"), t),
		}, true, true},
		{"x in (abc,)", LabelSelector{
			getRequirement("x", InOperator, NewStringSet("abc", ""), t),
		}, true, true},
		{"x in ()", LabelSelector{
			getRequirement("x", InOperator, NewStringSet(""), t),
		}, true, true},
		{"x notin (abc,,def),bar,z in (),w", LabelSelector{
			...
			getRequirement("x", NotInOperator, NewStringSet("abc", "", "def"), t),
			getRequirement("z", InOperator, NewStringSet(""), t),
		}, true, true},
		{"x,y in (a)", LabelSelector{
			getRequirement("y", InOperator, NewStringSet("a"), t),
			getRequirement("x", ExistsOperator, nil, t),
		}, false, true},
		{"x=a", LabelSelector{
			getRequirement("x", EqualsOperator, NewStringSet("a"), t),
		}, true, true},
		{"x=a,y!=b", LabelSelector{
			getRequirement("x", EqualsOperator, NewStringSet("a"), t),
			getRequirement("y", NotEqualsOperator, NewStringSet("b"), t),
		}, true, true},
		{"x=a,y!=b,z in (h,i,j)", LabelSelector{
			getRequirement("x", EqualsOperator, NewStringSet("a"), t),
			getRequirement("y", NotEqualsOperator, NewStringSet("b"), t),
			getRequirement("z", InOperator, NewStringSet("h", "i", "j"), t),
		}, true, true},
		...
		{"x notin (, a)", LabelSelector{
			getRequirement("x", NotInOperator, NewStringSet("", "a"), t),
		}, true, true},
		...
		{"a in (x,y,notin, z,in)", LabelSelector{
			getRequirement("a", InOperator, NewStringSet("in", "notin", "x", "y", "z"), t),
...
func getRequirement(key string, op Operator, vals StringSet, t *testing.T) Requirement {
...
func TestAdd(t *testing.T) {
	testCases := []struct {
	...
	}{
		{
			...
			InOperator,
			[]string{"value"},
			LabelSelector{Requirement{"key", InOperator, NewStringSet("value")}},
		},
		{
			LabelSelector{Requirement{"key", InOperator, NewStringSet("value")}},
			"key2",
			...
			[]string{"value2"},
			LabelSelector{
				Requirement{"key", InOperator, NewStringSet("value")},
				Requirement{"key2", EqualsOperator, NewStringSet("value2")},
			},
		},
	}
