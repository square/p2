...
type Pod interface {
	...
	Install(manifest.Manifest, auth.ArtifactVerifier, artifact.Registry, string, []string) error
	Uninstall() error
...
func (p *Preparer) installAndLaunchPod(pair ManifestPair, pod Pod, logger logging.Logger) bool {
	...
	err := pod.Install(pair.Intent, p.artifactVerifier, registry, p.containerRegistryAuthStr, p.dockerImageDirectoryWhitelist)
	if err != nil {
