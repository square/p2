...
func TestUpdatePods(t *testing.T) {
	...
	Assert(t).AreEqual(current[1], pods[0], "pod with id:1 should have been returned")
	Assert(t).AreEqual(current[2], pods[1], "pod with id:1 should have been returned")
	Assert(t).AreEqual("test", pods[2].manifest.Manifest.Id, "should have added pod with id:test to list")
}
...
func TestResultFromCheck(t *testing.T) {
	...
	val, _ := resultFromCheck(resp)
	Assert(t).AreEqual(health.Passing, val.Status, "200 should correspond to health.Passing")

	resp.StatusCode = 282
	val, _ = resultFromCheck(resp)
	Assert(t).AreEqual(health.Warning, val.Status, "2** should correspond to health.Warning")
