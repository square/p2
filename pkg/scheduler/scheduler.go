...
type Scheduler interface {
	EligibleNodes(store.Manifest, klabels.Selector) ([]store.NodeName, error)
}
...
func (sel *applicatorScheduler) EligibleNodes(_ store.Manifest, selector klabels.Selector) ([]store.NodeName, error) {
	nodes, err := sel.applicator.GetMatches(selector, labels.NODE, false)
	if err != nil {
		return nil, err
	}

	result := make([]store.NodeName, len(nodes))
	for i, node := range nodes {
		result[i] = store.NodeName(node.ID)
	}
	return result, nil
}
