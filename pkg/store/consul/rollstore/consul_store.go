...
type RollLabeler interface {
	...
	SetLabelsTxn(txn *api.KVTxnOps, labelType labels.Type, id string, labels map[string]string) error
...
type ReplicationControllerStore interface {
	...
	CreateTxn(txn *api.KVTxnOps, manifest manifest.Manifest, nodeSelector klabels.Selector, podLabels klabels.Set) (rc_fields.RC, error)
...
func (s ConsulStore) lockRCs(rcIDs rc_fields.IDs, session consul.Session) (*api.KVTxnOps, error) {
	retTxn := new(api.KVTxnOps)
	sort.Sort(rcIDs)
	for _, rcID := range rcIDs {
		...
		*retTxn = append(*retTxn, &api.KVTxnOp{
...
func (s ConsulStore) CreateRollingUpdateFromExistingRCs(u roll_fields.Update, newRCLabels klabels.Set, rollLabels klabels.Set) (roll_fields.Update, error) {
	...
	txn, err := s.lockRCs(rcIDs, session)
	if err != nil {
		return roll_fields.Update{}, err
	}
...
func (s ConsulStore) CreateRollingUpdateFromOneExistingRCWithID(
	...
	*txn = append(*txn, []*api.KVTxnOp(*newRCIsLockedTxn)[0])
...
func (s ConsulStore) CreateRollingUpdateFromOneMaybeExistingWithLabelSelector(
	...
	*txn = append(*txn, []*api.KVTxnOp(*oldRCIsLockedTxn)[0])
...
func (s ConsulStore) createRU(
	...
	*txn = append(*txn, &api.KVTxnOp{
	...
	ok, resp, _, err := s.kv.Txn(*txn, nil)
	if err != nil {
		return util.Errorf("ru creation transaction failed: %s", err)
	}

	if len(resp.Errors) != 0 {
		return util.Errorf("some errors occurred when applying the RU creation transaction: %s", resp.Errors)
	}

	// I think ok being false means there should be something in resp.Errors, so this
	// should be impossible.
	if !ok {
		return util.Errorf("an unknown error occurred when applying the RU creation transaction")
	}
